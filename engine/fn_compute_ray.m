    ...
    for tx = 1 : probe_els
        ...
        % use the Dijkstra method to calculate the fermat path.
            ...
            for ii = 1 : wall_pixels
                ...
                min_times(ii, 1) = ( ...
                    sqrt((geometries(1, ii, 1) - probe_coords(tx, 1)) ^ 2 + ...
                         (geometries(1, ii, 3) - probe_coords(tx, 3)) ^ 2) / speeds(1) ...
                );
            end
                    ...
                    for ii = 1 : wall_pixels
                        for jj = 1 : wall_pixels
                            min_times_matrix(ii, jj) = min_times(ii, 1) + ( ...
                                sqrt((geometries(wall, jj, 1) - geometries(wall-1, ii, 1)) ^ 2 + ...
                                     (geometries(wall, jj, 3) - geometries(wall-1, ii, 3)) ^ 2) / speeds(wall) ...
                            );
                        end
                    end
            ...
            for ii = 1 : wall_pixels
                min_times(ii, 1) = min_times(ii, 1) + ( ...
                    sqrt((scatterers(scat, 1) - geometries(no_walls, ii, 1)) ^ 2 + ...
                         (scatterers(scat, 3) - geometries(no_walls, ii, 3)) ^ 2) / ...
                    speeds(no_walls + 1) ...
                );
            end
