...
if walls == 0 % We are in the contact case.
    ...
    [no_walls, wall_pixels, ~] = size(geometries);
end
    ...
    for tx = 1 : probe_els
        ...
        % use the Dijkstra method to calculate the fermat path.
            ...
            for i = 1 : wall_pixels
                ...
                min_times(i, 1) = ( ...
                    sqrt((geometries(1, i, 1) - probe_coords(tx, 1)) ^ 2 + ...
                         (geometries(1, i, 3) - probe_coords(tx, 3)) ^ 2) / speeds(1) ...
                );
            end
            ...
            if no_walls > 1
                ...
                for wall = 2 : no_walls
                    ...
                    for i = 1 : wall_pixels
                        for j = 1 : wall_pixels
                            min_times_matrix(i, j) = min_times(i, 1) + ( ...
                                sqrt((geometries(wall, j, 1) - geometries(wall-1, i, 1)) ^ 2 + ...
                                     (geometries(wall, j, 3) - geometries(wall-1, i, 3)) ^ 2) / speeds(wall) ...
                            );
                        end
                    end
            ...
            for i = 1 : wall_pixels
                min_times(i, 1) = min_times(i, 1) + ( ...
                    sqrt((scatterers(scat, 1) - geometries(no_walls, i, 1)) ^ 2 + ...
                         (scatterers(scat, 3) - geometries(no_walls, i, 3)) ^ 2) / ...
                    speeds(no_walls + 1) ...
                );
            end
