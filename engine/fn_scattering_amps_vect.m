...
if scat_info.type == "point"
    ...
    scat_amps = ones(probe_els^2, num_scatterers, num_freqs) * ...
                fn_point_scatterer_amp( ...
                    inc_mode, out_mode, v_L, v_T ...
                );
elseif scat_info.type == "sdh"
    if isfield(scat_info, 'matrix') % If matrices have been precomputed.
        inc_angle_on_scat = path1.weights.inc_theta(:, :, end, 2) - scat_info.angle;
        out_angle_on_scat = path2.weights.inv_out_theta(:, :, 1, 2) - scat_info.angle;
...
elseif scat_info.type == "image"
    ...
    error("fn_scattering_amps: invalid scatterer type")
end
