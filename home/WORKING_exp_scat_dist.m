...
if load_in_data
    ...
    nu_temp = mean(nd, 1);
    nd_temp = real(nd .* exp(-1j*angle(nu_temp)));
    sigma = squeeze(std(nd_temp, 0, 1) ./ sqrt(2));
    nu = squeeze(mean(abs(nd), 1));
    for pt = 1:size(de, 3)
        for im = 1:size(de, 2)
            try
                %% Do the fitting
                fitted = fitdist(abs(nd(:, im, pt)), 'Rician');
                fitted = makedist("Rician", "s", nu(im, pt), "sigma", sigma(im, pt));
...
if plot_everything
    ...
    grp(1).Label.String = "\nu";
    savefig(fullfile(dirname, sprintf("Nu %s Non-Defective Relative Coords%s.fig", b_or_s, suffix)))
    ...
    savefig(fullfile(dirname, sprintf("Sigma %s Non-Defective Relative Coords%s.fig", b_or_s, suffix)))
...
%% Plot goodness of fit parameters
    ...
    savefig(fullfile(dirname, sprintf("KS p-val %s%s.fig", b_or_s, suffix)))
...
%     savefig(fullfile(dirname, sprintf("Chi2 p-val %s%s.fig", b_or_s, suffix)))
    ...
    savefig(fullfile(dirname, sprintf("KS Statistic %s%s.fig", b_or_s, suffix)))
...
%     savefig(fullfile(dirname, sprintf("Chi2 statistic %s%s.fig", b_or_s, suffix)))
end

disp("starting fusion")
...
%                 cd(thisdir)
                ...
                yaml_options.model.savename = sprintf("%s%s", matname, suffix);
%                 fn_fisher_fusion(yaml_options, Ims, Nu, Sigma, KS);
                Fused = fn_fisher_teststat(yaml_options, Ims, Sigma);
                max_ = 0;
                if max(abs(Fused.image(:))) > max_
...
%                 cd(thisdir)
                ...
                savefig(fullfile(thisdir, sprintf("%s View-wise Likelihood%s.fig", matname, suffix)))
